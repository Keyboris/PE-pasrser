...
class SectionFeatureExtractor:
    ...
    def high_entropy_section_count(self):
        e = self.entropies
        return sum(entropy > 7 for entropy in e)

    def executable_sections_count(self):
        count = 0
    ...
    def writeable_executable_sections_count(self):
        count = 0
        for section_header in self.section_headers:
    ...
    def sections_with_zero_raw_size_count(self):
        return sum(header.SizeOfRawData == 0 for header in self.section_headers)

    def largest_section_size_ratio(self):
    ...
    def extract_analysis_features(self):
        analysis_features = {
